...
@Singleton class ChatController @Inject() (mcc: MessagesControllerComponents, authClient: AuthServiceClient, chatClient: ChatServiceClient)(implicit ec: ExecutionContext) extends MessagesAbstractController(mcc) with SiteControllerHelpers {
  ...
  def submitMessage = Action.async { implicit request: MessagesRequest[AnyContent] =>
    withSessionCookieId(request) { sessionId =>
  ...
  private def chatRoom(sessionId: String, form: Form[ChatRequest] = chatForm)(implicit msgs: Messages): Future[Result] =
